...
module.exports = (db) => {
  ...
  router.get("/:user_id/sessions/:session_id", (req, res) => {
    ...
    db.query("SELECT sessions.*, users.name as mentor_name FROM sessions JOIN mentors on mentors.id = sessions.mentor_id JOIN users ON users.id = mentors.user_id WHERE users.id = $1 AND sessions.id = $2", [user_id, session_id]).then((data) => {
      res.json(data.rows)
    })
  })
  ...
  router.post("/:user_id/sessions/:session_id", (req, res) => {
    ...
    db.query("INSERT INTO mentor_reviews (rating, message) VALUES ($1, $2) RETURNING *", [rating, message]).then((data) => {
      res.json(data.rows)
    })
  })
